...
class Trainer:
    def __init__(self, train_ds, test_ds):
        ...
        self.model = TSCNet(num_channel=32, num_features=self.n_fft // 2 + 1).cuda()
    ...
    def train_step(self, batch):
        ...
        # Normalization
        c = torch.sqrt(noisy.size(-1) / torch.sum((noisy ** 2.0), dim=-1) + 1e-8)
        ...
        noisy_spec_input = noisy_spec.permute(0, 1, 3, 2)
        clean_spec = power_compress(clean_spec)
        clean_real = clean_spec[:, 0, :, :].unsqueeze(1)
        clean_imag = clean_spec[:, 1, :, :].unsqueeze(1)

        est_real, est_imag, cmask = self.model(noisy_spec_input)
    ...
    def train(self):
        ...
        for epoch in range(args.epochs):
            ...
            gen_loss = self.test()
            path = os.path.join(args.save_model_dir, 'D2Former_epoch_' + str(epoch) + '_' + str(gen_loss)[:5])
