...
def evaluation(model_path, noisy_dir, clean_dir, save_tracks, saved_dir, out_file):
    f = open(out_file, 'a')
    f.write(model_path+'\n')
    n_fft = 400
    ...
    for audio in audio_list:
        ...
        print('pesq: ', metrics[0], 'csig: ', metrics[1], 'cbak: ', metrics[2], 'covl: ',
          metrics[3], 'ssnr: ', metrics[4], 'stoi: ', metrics[5])
    ...
    f.write('pesq: '+str(metrics_avg[0])+ ' csig: '+ str(metrics_avg[1]) + ' cbak: '+str(metrics_avg[2]) + ' covl: '+str(metrics_avg[3])+' ssnr: '+str(metrics_avg[4])+ ' stoi: '+str(metrics_avg[5]) + '\n')
    f.close()
